...
def node_section(struct):
    ...
    for field in struct.fields:
        if isinstance(field, Union):
            ...
            rows.append(['', f'{field.__class__.__name__}[{", ".join(markdown_section_link(s.__name__) for s in field.structs)}]', '', ''])
        elif isinstance(field, Vector):
            if field.klass not in structs:
                structs.add(field.klass)
            rows.append(['', field.__class__.__name__, '', markdown_section_link(field.klass.__name__)])

        elif isinstance(field, Field):
            rows.append([field.name, field.__class__.__name__, field.nbits, field.help or ""])
